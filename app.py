    ...
    while True:
        ...
        if hand_results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(hand_results.multi_hand_landmarks,
                ...
                y_da_mao_size = [calc_euclidian_distance(landmark_list[0], landmark_list[17])]
                x_da_mao_size =  [calc_euclidian_distance(landmark_list[5], landmark_list[17])]

                point_history[hand_side].append(landmark_list[0] + y_da_mao_size + x_da_mao_size)
            ...
            cm_timer += 1
            [point_history[side].append([0, 0, 0, 0]) for side in ("L","R") if side not in hand_side_history]
        else:
...
def pre_process_point_history(image, point_history):
    ...
    for index, point in enumerate(temp_point_history):
        ...
        temp_point_history[index][2] = (temp_point_history[index][2] -
                                        base_zy) / image_height
        temp_point_history[index][3] = (temp_point_history[index][3] -
                                        base_zx) / image_height
...
def draw_landmarks(image, landmark_point):
    if len(landmark_point) > 0:
        ...
        cv.line(image, tuple(landmark_point[17]), tuple(landmark_point[0]),
                (20, 180, 240), 2)
        cv.line(image, tuple(landmark_point[17]), tuple(landmark_point[5]),
                (0, 0, 0), 6)
        cv.line(image, tuple(landmark_point[17]), tuple(landmark_point[5]),
                (20, 180, 240), 2)
...
def draw_info_text(image, brect, hand_side, 
                    finger_gesture_text, 
                   probability_rank):
